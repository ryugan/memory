...
class Board {
	...
	render() {
		...
		const cardsNumber = cards === 'undefined' ? 0 : cards.length;
		...
		var lastHalfCardIndex = halfCardIndex -1;
		...
		for (var cpt = 0; cpt < 2; cpt++) {
			...
			for (var cpt2 = 0; cpt2 < cardsNumber; cpt2++) {
				...
				// Si on est en première position ou à la moitié de la liste, on créée une ligne
				if (cpt2 === 0 || cpt2 === halfCardIndex) {
				...
				// Si on est dernière position ou à la fin de la liste, on ferme la ligne
				if (cpt2 === lastCardIndex || cpt2 === lastHalfCardIndex) {
					result += '<div class=\'row\'>
				}
			}
		}
